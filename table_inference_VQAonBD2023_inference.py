...
import numpy as np
import torch
from mmcv.image import gray2bgr, imread, imwrite
        ...
        try:
            if master_token_list[pointer] == "<td":
                if (pointer + 5) <= len(master_token_list) and (
                    master_token_list[pointer + 2].startswith(" colspan=")
                    ...
                    master_token_list[pointer + 1].startswith(" colspan=")
        ...
        if master_token.startswith("<td"):
            if text_count > len(cell_content_list) - 1:
                ...
                master_token = master_token.replace(
                    "><", ">{}<".format(cell_content_list[text_count])
                )
                text_count += 1
...
def pp_table_recognition(result_dict, path_html):
    ...
    with open(
        os.path.join(path_html, file_name.replace(".png", ".txt")), "w"
    ) as f_html:
    ...
    def do_structure_predict(self, path, is_save=True, gpu_idx=None):
        if isinstance(path, str):
            if os.path.isfile(path):
                ...
                _, result_dict = self.master_structure_inference.predict_single_file(
                    path
                )
                ...
                for file in tqdm(files):
                    (
                        _,
                        result_dict,
                    ) = self.master_structure_inference.predict_single_file(file)
                    pp_table_recognition(
                        result_dict, self.structure_master_result_folder
                    )
            ...
            for i, p in enumerate(path):
                try:
                    (
                        _,
                        result_dict,
                    ) = self.master_structure_inference.predict_single_file(p)
                    pp_table_recognition(
                        result_dict, self.structure_master_result_folder
                    )
                    if gpu_idx is not None:
                        print(
                            "[GPU_{} : {} / {}] {} file structure inference. ".format(
                                gpu_idx, i + 1, len(path), p
                            )
                        )
                    else:
...
if __name__ == "__main__":
    ...
    cfg = {
        ...
        # check point file
        "structure_master_ckpt": "/home2/nam/nam_data/work_dir/1114_TableMASTER_local_attn_new_decoder_FinTabNet_alldata_img520_win300_0_tag600_cell150_batch4/epoch_20.pth",  # structure
